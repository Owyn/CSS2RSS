...
* for everything (even for the `1) item` - `$` at start - executes (via `eval()`) a python code expression instead of using CSS selectors, the return value (can be an array if you're using `@` for `1) item`) of that expression will be used for that item e.g. `$found_link.select_one('img')['alt']` would return `alt` text from an `img` element inside found link, see https://www.crummy.com/software/BeautifulSoup/bs4/doc/ for things you can do with the soup - e.g. go one level up (to the parent element) or to the next element - or select elements CSS selectors can't select, or anything you can do with Python - see examples section below, useful script's variables are `found_link` and `item` (found link and the whole found item) - you can CSS `.select` or `.select_one` inside them e.g. - `$item.select_one('img.class')` to find an image element of class "class" inside your found root item, or use `soup` var - `$str(soup.select_one('img.class'))` to select an element globally (and make into a text - we need text for the descriptio)
