...
if found_items_n != 0:
  for item in found_items:
    css_to_rss(item, 0)
  json_feed = "{{\"title\": {title}, \"version\": {jsonfeed_version}, \"description\": {description}, \"items\": [{items}]}}"
  json_feed = json_feed.format(title = json.dumps(soup.title.text), jsonfeed_version = json.dumps(jsonfeed_version), description = json.dumps("Script found "+str(found_items_n)+" items"+str(description_addon)) if found_items_bad_n == 0 else json.dumps("Script found "+str(found_items_n)+" items, " + str(found_items_bad_n) + " bad items with no link"+str(description_addon)), items = ", ".join(items))
else:
  items.append("{{\"title\": {title}, \"content_html\": {html}, \"url\": {url}}}".format(
    ...
    url=json.dumps("")))
  json_feed = "{{\"title\": {title}, \"version\": {jsonfeed_version}, \"description\": {description}, \"items\": [{items}]}}"
  json_feed = json_feed.format(title = (json.dumps("ERROR: " + soup.title.text) if soup.title else json.dumps("ERROR")), jsonfeed_version = json.dumps(jsonfeed_version), description = json.dumps("Error: - CSS selector found no items"), items = ", ".join(items))
